...
def render_experiment_setup_section() -> ExperimentParams:
    ...
    with st.form("experiment_setup"):
        ...
        submit = st.form_submit_button("Confirm Parameters")
        if submit:
            return params
        else:
            return None
...
def render_experiment_results(params: ExperimentParams):
    """
    Renders the results of the experiment based on the provided parameters.
    """
    # Display hypothesis
    if params:
        ...
        st.info(f"Expected treatment conversion rate: {params.treatment_rate():.2%} (a {params.mde*100:.1f}% increase)")
    else:
        st.warning("Please enter the experiment parameters before running the experiment.")
